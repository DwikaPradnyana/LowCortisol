      ...
      <div className="w-full max-w-lg">
        ...
        <div className="relative mb-6 px-2 sm:px-4">
            ...
            <div className="relative z-10 flex items-center justify-between">
                {stepsConfig.map((s) => (
                    <div key={s.num} className="flex flex-col items-center gap-1.5 sm:gap-2 w-16 sm:w-20">
                        <div className={`flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full transition-all duration-500 ${ step >= s.num ? "bg-primary text-white shadow-md shadow-primary/30" : "bg-slate-200 text-slate-400" }`} >
                            <s.icon weight={step >= s.num ? "bold" : "regular"} className="h-4 w-4 sm:h-5 sm:w-5" />
                        </div>
        ...
        {/* PERBAIKAN: Padding responsif p-5 untuk mobile, p-8 untuk layar besar */}
        <GlassCard className="p-5 sm:p-8 shadow-xl">
          ...
          {errorMsg && (
            <div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-3 text-xs sm:text-sm font-medium text-red-700 animate-in fade-in">
              {errorMsg}
            </div>
          )}
          ...
          <div className="space-y-4 sm:space-y-5 animate-in fade-in slide-in-from-right-4 duration-500">
            {step === 1 && (
              <>
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Jenis Kelamin</label>
                  <div className="flex gap-2">
                    {['Laki-laki', 'Perempuan'].map(opt => (
                      <button key={opt} onClick={() => setPillValue('jenis_kelamin', opt)}
                        className={`flex-1 py-2.5 sm:py-3 px-1 text-xs sm:text-sm font-medium rounded-xl border transition-all ${
                ...
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Usia (Tahun)</label>
                  <input type="number" name="usia" value={formData.usia} onChange={handleInputChange} placeholder="Minimal 15" min="15" max="70"
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20" />
                </div>

                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Pendidikan Terakhir</label>
                  <select name="pendidikan_terakhir" value={formData.pendidikan_terakhir} onChange={handleInputChange}
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 cursor-pointer">
                ...
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Status Pernikahan</label>
            ...
            {step === 2 && (
              <>
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Tipe Perusahaan</label>
                  <select name="tipe_perusahaan" value={formData.tipe_perusahaan} onChange={handleInputChange}
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary cursor-pointer">
                ...
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Departemen</label>
                ...
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Lama Bekerja (Tahun)</label>
                  <input type="number" name="lama_bekerja_tahun" value={formData.lama_bekerja_tahun} onChange={handleInputChange} placeholder="Contoh: 2" min="0" max="50"
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary" />
                </div>

                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Status WFH</label>
                  <div className="flex gap-2">
                    {['Ya', 'Tidak', 'Hybrid'].map(opt => (
                      <button key={opt} onClick={() => setPillValue('status_wfh', opt)}
                        className={`flex-1 py-2.5 sm:py-3 px-1 text-[11px] sm:text-sm font-medium rounded-xl border transition-all ${
            ...
            {step === 3 && (
              <>
                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Keamanan Pekerjaan</label>
                  <select name="keamanan_pekerjaan" value={formData.keamanan_pekerjaan} onChange={handleInputChange}
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary cursor-pointer">
                    <option value="" disabled>Apakah posisi terancam?</option>
                    {['Tidak Aman', 'Cukup Aman', 'Aman', 'Sangat Aman'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>

                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Kesehatan Mental</label>
                  <select name="riwayat_kesehatan_mental" value={formData.riwayat_kesehatan_mental} onChange={handleInputChange}
                    className="w-full rounded-xl border border-slate-200 bg-white/50 py-2.5 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm outline-none transition focus:border-primary cursor-pointer">
                    <option value="" disabled>Diagnosa klinis (jika ada)</option>
                    {['Tidak Ada', 'Pernah Depresi', 'Kecemasan'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>

                <div>
                  <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 sm:mb-2 block">Status Merokok</label>
                  <div className="flex gap-2">
                ...
                <div>
                  <div className="flex justify-between mb-1.5 sm:mb-2">
                    <label className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground block">Frekuensi Konflik Kerja</label>
                    <span className="text-[10px] sm:text-xs font-bold text-primary">{formData.frekuensi_konflik_kerja} / 10</span>
                  </div>
                  ...
                  <div className="flex justify-between text-[9px] sm:text-[10px] text-muted-foreground mt-1.5 font-medium">
                    <span>Harmonis</span>
          ...
          <div className="mt-6 sm:mt-8 flex gap-2 sm:gap-3">
            {step > 1 && (
              <Button type="button" variant="outline" onClick={prevStep} disabled={isLoading} className="px-4 sm:px-5">
                <CaretLeft weight="bold" className="h-4 w-4 sm:h-5 sm:w-5" />
              </Button>
            )}
            
            {step < 3 ? (
              <Button type="button" onClick={nextStep} className="flex-1 flex justify-center items-center gap-2 py-2.5 sm:py-3">
                Continue <CaretRight weight="bold" className="h-4 w-4 sm:h-5 sm:w-5" />
              </Button>
            ) : (
              <Button type="button" onClick={handleSubmit} disabled={isLoading} className="flex-1 flex justify-center items-center gap-2 py-2.5 sm:py-3 text-sm sm:text-base">
                {isLoading ? (
                  <><CircleNotch className="h-4 w-4 sm:h-5 sm:w-5 animate-spin" /> Saving...</>
                ) : (
                  <><CheckCircle weight="bold" className="h-4 w-4 sm:h-5 sm:w-5" /> Finish Setup</>
                )}
              </Button>
            )}
          </div>
